...
import { MasonryStyle } from "comps/controls/styleControlConstants";
...
const childrenMap = {
  ...
  noOfCols: withDefault(RangeControl.closed(0, 12), 3),
  ...
  style: styleControl(MasonryStyle),
};

type ViewProps = RecordConstructorToView<typeof childrenMap>;
type Props = ViewProps & { dispatch: DispatchType };

const ContainerInMasonryView = (props: ContainerBaseProps) => {
  return (
    <InnerGrid
      {...props}
      emptyRows={15}
...
const MasonryView = (props: Props) => {
  ...
  const isOneRow =
    props.noOfRows > 0 &&
    (_.size(containerProps.layout) === 0 || editorState.isDragging);
    ...
    const render = (
      <div
        ...
        style={{
          height: rowHeight,
          border:
            props.style.cardBorder && `1px solid ${props.style.cardBorder}`,
          borderRadius: props.style.cardRadius,
          marginBottom: props.style.gap,
          backgroundColor: props.style.cardColor,
        }}
      >
        <BackgroundColorContext.Provider value={props.style.background}>
          <ContainerInMasonryView
  ...
  return (
    <div
      ...
      style={{
        ...
        columnCount: props.noOfCols || 3,
        columnGap: props.style.gap,
      }}
    >
      <ReactResizeDetector
      ...
      >
        <div style={{ height: props.autoHeight ? "auto" : "100%" }}>
          {renders}
        </div>
      </ReactResizeDetector>
    </div>
...
const MasonryViewTmpComp = new UICompBuilder(childrenMap, (props, dispatch) => (
  <MasonryView {...props} dispatch={dispatch}></MasonryView>
))
  .setPropertyViewFn((children) => (
    <>
      <Section name={sectionNames.basic}>
        {children.noOfRows.propertyView({
          label: trans("masonryView.numOfRows"),
          tooltip: trans("masonryView.numOfRowsTooltip"),
        })}

        {children.noOfCols.propertyView({
          label: trans("masonryView.numOfCols"),
          tooltip: trans("masonryView.numOfColsTooltip"),
        })}
...
class MasonryViewImplComp extends MasonryViewTmpComp implements IContainer {
  ...
  updateContext(i: number): this {
    return this.reduce(
      wrapChildAction(
        "container",
        ContextContainerComp.changeContextDataAction({ i })
      )
    );
  }
  override reduce(action: CompAction): this {
    ...
    return reduceInContext({ inEventContext: true }, () =>
      super.reduce(action)
    );
  }
}

export const MasonryViewComp = withExposingConfigs(MasonryViewImplComp, [
  NameConfigHidden,
]);

export function defaultMasonryData(
  compName: string,
